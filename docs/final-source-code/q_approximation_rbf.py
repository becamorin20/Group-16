...
#
# Implements Q-learning algorithm with function approximation, 9 stationary rbf gaussian circular
...
def p5( agent, z1, z2, z3 ):
    ...
    total_zombie = in_partition(z1) + in_partition(z2) + in_partition(z3)
    
    #add weight for wall: 0.25
    total_wall = 0
    if agent[0]>-2 or agent[0]<-9 or agent[1]>3 or agent[1]<-4:
        total_wall = 0.25
    
    return total_wall + total_zombie
...
def a1( agent ):
    ''' Gaussian with center (-2.5,2.5) and std deviation 1.25  '''
    x = agent[0]
    z = agent[1]
    dist = (-2.5-x)**2. + (2.5-z)**2.
    var = 1.25**2.
    return math.exp( -1 * dist / (2.*var) ) 
    
def a2( agent ):
    ''' Gaussian with center (-5.5,2.5) and std deviation 1.25 '''
    x = agent[0]
    z = agent[1]
    dist = (-5.5-x)**2. + (2.5-z)**2.
    var = 1.25**2.
    return math.exp( -1 * dist / (2.*var) ) 
    
def a3( agent ):
    ''' Gaussian with center (-8.5,2.5) and std deviation 1.25 '''
    x = agent[0]
    z = agent[1]
    dist = (-8.5-x)**2. + (2.5-z)**2.
    var = 1.25**2.
    return math.exp( -1 * dist / (2.*var) ) 

def a4( agent ):
    ''' Gaussian with center (-2.5,-0.5) and std deviation 1.25 '''
    x = agent[0]
    z = agent[1]
    dist = (-2.5-x)**2. + (-0.5-z)**2.
    var = 1.25**2.
    return math.exp( -1 * dist / (2.*var) ) 
    
def a5( agent ):
    ''' Gaussian with center (-5.5,-0.5) and std deviation 1.25 '''
    x = agent[0]
    z = agent[1]
    dist = (-5.5-x)**2. + (-0.5-z)**2.
    var = 1.25**2.
    return math.exp( -1 * dist / (2.*var) ) 

def a6( agent ):
    ''' Gaussian with center (-8.5,-0.5) and std deviation 1.25 '''
    x = agent[0]
    z = agent[1]
    dist = (-8.5-x)**2. + (-0.5-z)**2.
    var = 1.25**2.
    return math.exp( -1 * dist / (2.*var) ) 

def a7( agent ):
    ''' Gaussian with center (-2.5,-3.5) and std deviation 1.25 '''
    x = agent[0]
    z = agent[1]
    dist = (-2.5-x)**2. + (-3.5-z)**2.
    var = 1.25**2.
    return math.exp( -1 * dist / (2.*var) ) 
    
def a8( agent ):
    ''' Gaussian with center (-5.5,-3.5) and std deviation 1.25 '''
    x = agent[0]
    z = agent[1]
    dist = (-2.5-x)**2. + (-3.5-z)**2.
    var = 1.25**2.
...
def a9( agent ):
    ''' Gaussian with center (-8.5,-3.5) and std deviation 1.25 '''
    x = agent[0]
    z = agent[1]
    dist = (-2.5-x)**2. + (-3.5-z)**2.
    var = 1.25**2.
...
class Agent(object):
    ...
    def get_basis( self, agent, z1, z2, z3, action ):
        ...
        for i in range(4):
            if self.possible_actions[i]==action:
                self.basis[0 + i*18] = p1(agent, z1, z2, z3) 
                self.basis[1 + i*18] = p2(agent, z1, z2, z3) 
                self.basis[2 + i*18] = p3(agent, z1, z2, z3) 
                self.basis[3 + i*18] = p4(agent, z1, z2, z3) 
                self.basis[4 + i*18] = p5(agent, z1, z2, z3) 
                self.basis[5 + i*18] = p6(agent, z1, z2, z3) 
                self.basis[6 + i*18] = p7(agent, z1, z2, z3) 
                self.basis[7 + i*18] = p8(agent, z1, z2, z3) 
                self.basis[8 + i*18] = p9(agent, z1, z2, z3) 
                self.basis[9 + i*18] = a1(agent) 
                self.basis[10 + i*18] = a2(agent) 
                self.basis[11 + i*18] = a3(agent)
                self.basis[12 + i*18] = a4(agent)
                self.basis[13 + i*18] = a5(agent)
                self.basis[14 + i*18] = a6(agent)
                self.basis[15 + i*18] = a7(agent) 
                self.basis[16 + i*18] = a8(agent)
                self.basis[17 + i*18] = a9(agent)
        '''for i in range(4):
            print self.basis[9 + i*18],
            print self.basis[10 + i*18],
            print self.basis[11 + i*18],
            print self.basis[12 + i*18],
            print self.basis[13 + i*18],
            print self.basis[14 + i*18],
            print self.basis[15 + i*18],
            print self.basis[16 + i*18],
            print self.basis[17 + i*18] '''
        return self.basis
    ...
    def update_hyperparameters(self):
        ...
        eps_evolution = { 1:0.1, 50:0.05, 75:0.01, 90:0.005 }
        alpha_evolution = { 1:0.05, 25:0.01, 50:0.005, 75:0.001 }
    ...
    def run(self, agent_host):
        ...
        self.update_parameters_from_terminal( self.action, -10. )
